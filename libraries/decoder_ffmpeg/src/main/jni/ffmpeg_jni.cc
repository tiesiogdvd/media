...
struct DsdContext {
  bool isDsd;
  int dsdSampleRate;
  SwrContext *resampleContext; // Store the resampler here
  
  // Constructor to initialize fields
  DsdContext() : isDsd(false), dsdSampleRate(0), resampleContext(nullptr) {}
};
...
AUDIO_DECODER_FUNC(jint, ffmpegGetSampleRate, jlong context) {
  if (!context) {
    LOGE("Context must be non-NULL.");
    return 44100; // Return a safe default rather than -1
  }
  ...
  // Validate the sample rate regardless of format
  if (detectedRate < 8000 || detectedRate > 192000) {
    LOGE("Invalid sample rate detected: %d Hz, using default 44100 Hz", detectedRate);
...
AUDIO_DECODER_FUNC(jlong, ffmpegReset, jlong jContext, jbyteArray extraData) {
  ...
  if (dsdContext && dsdContext->isDsd) {
    isDsd = true;
    LOGD("Resetting DSD codec context");
  }
...
const AVCodec *getCodecByName(JNIEnv *env, jstring codecName) {
  ...
  LOGD("Looking for codec by name: %s", codecNameChars);
...
AVCodecContext *createContext(JNIEnv *env, const AVCodec *codec,
  ...
  if (isDsd) {
    ...
    dsdContext->dsdSampleRate = DSD_OUTPUT_SAMPLE_RATE; // Use standard rate
...
int decodeDsdPacket(AVCodecContext *context, AVPacket *packet,
  ...
  // Validate DsdContext
  if (!dsdContext) {
    LOGE("DsdContext is null, cannot decode DSD packet");
    return AUDIO_DECODER_ERROR_OTHER;
  }
...
void releaseContext(AVCodecContext *context) {
  ...
  // Check if it's a DSD context
  DsdContext *dsdContext = static_cast<DsdContext*>(context->opaque);
  if (dsdContext) {
    // It's a DSD context
    if (dsdContext->isDsd) {
      ...
      // Free the DSD context
      delete dsdContext;
      context->opaque = NULL;
    } else {
      // Not actually a DSD context, treat as SwrContext
